...
const TIER_ARMS: { heightFrac: number; width: number }[] = [
  { heightFrac: 0.5, width: 1.8 },
...
export class Tower {
  ...
  private capacity(): number {
    return ECONOMY.towerTierCapacity[this.tier - 1];
  }
  ...
  private addArmForTier(tier: number): void {
    const arm = TIER_ARMS[tier - 1];
    if (!arm) return;
  ...
  upgrade(): void {
    ...
    this.addArmForTier(this.tier);
    this.tier++;
    this.upgradePulseStart = performance.now();
  }

  /** Applies saved tier/connection/pending state instantly, skipping spawn and upgrade animations. */
  materializeFromSave(tier: number, connections: number, pendingMs?: number): void {
    this.settled = true;
    this.group.scale.setScalar(1);
    for (let t = 1; t < tier; t++) this.addArmForTier(t);
    this.tier = tier;
